...
export async function insertAppDeployment(
  appDeploymentObj: typeof appDeploymentSchema,
) {
...
export async function insertDeploymentContainers(
  deploymentContainersObj: typeof deploymentContainersSchema,
) {
  return await db.insert(deploymentContainersSchema).values(
    deploymentContainersObj,
  );
}
...
export async function updateDeploymentContainersStatus(
  ...
  return await db.update(deploymentContainersSchema).set({ status }).where(
    eq(deploymentContainersSchema.deploymentId, deploymentId),
  );
}
