...
CROSS JOIN (
    ...
    WHERE p.name = 'Provider One'
) pr
WHERE d.name = 'Dev Alice';

-- Developer App Containers
INSERT INTO developer_app_containers (
    developer_id,
    provider_resource_id,
    container_id,
    app_name,
...
CROSS JOIN (
    ...
    FROM provider_resources pr
    JOIN providers p ON p.id = pr.provider_id
    WHERE p.name = 'Provider Two'
) pr
WHERE d.name = 'Dev Bob';
